...
void TalkWindowShell::processPendingData()
{
	...
	while(m_udpReceiver->hasPendingDatagrams())
	{
		...
		QString strData = btData.data();//收到的数据
		QString strWindowID;			//聊天窗口ID(①群聊则是群号②单聊则是员工QQ号)
		QString strSendEmployeeID;		//发送端的QQ号
		...
		if (btData[0] == '1')
		{
			...
			//★文本信息:群聊标志+发信息员工QQ号+群号+信息类型（1）+数据长度+数据
			if (cMsgType == '1')		
			{
				//数据类型(文本)
				msgType = 1;
			...
			//★表情信息:群聊标志+发信息员工QQ号+群号+信息类型（0）+表情个数+images+表情名称
			else if (cMsgType == '0')	
			{
				//数据类型(表情)
				msgType = 0;
		...
		else
		{
			...
			//▲文本信息:群聊标志+发信息员工QQ号+收信息员工QQ号+信息类型（1）+数据长度+数据
			if (cMsgType == '1')
			{
				msgType = 1;
			...
			//▲表情信息:群聊标志+发信息员工QQ号+收信息员工QQ号 + 信息类型（0）+表情个数+images+表情名称
			//
			else if (cMsgType == '0')
			{
				msgType = 0;
			...
			//▲文件信息:群聊标志 + 发信息员工QQ号 + 收信息员工QQ号（群QQ号） + 信息类型（2） 
